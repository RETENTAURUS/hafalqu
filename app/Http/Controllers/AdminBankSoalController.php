...
class AdminBankSoalController extends Controller
{
    ...
    public function storeJuz(Request $request)
    {
        ...
        return redirect()->route('admin.soal.index')->with('success', 'Juz baru berhasil ditambahkan!');
    }
    ...
    public function showJuz(Request $request, $juz_id)
    {
        ...
        return view('admin.bank-soal-surat', compact('juz', 'suratAktif'));
    }
    ...
    public function storeSurat(Request $request, $juz_id)
    {
        ...
        return redirect()->route('admin.soal.showJuz', $juz_id)->with('success', 'Surat baru berhasil ditambahkan!');
    }
    ...
    public function showSurat($surat_id)
    {
        ...
        return view('admin.bank-soal-list-soal', compact('surat'));
    }
...
public function storeSoal(Request $request)
{
    $request->validate([
        ...
        'jenis'         => 'required|in:melanjutkan,mengisi,pengetahuan,audio', // <- diperbaiki
 ...
 public function showSoal($id) // <-- Ubah dari show ke showSoal
{
    ...
    return response()->json([
        ...
        'poin'          => $soal->poin ?? 100,
        'file_audio'    => $soal->file_audio ? asset('storage/audio_soals/' . $soal->file_audio) : null, // <-- Perbaiki path panggil asset
        'surat'         => $soal->surat?->nama_surat,
    ]);
}
 
public function editSoal($id) // <-- Ubah dari edit ke editSoal
{
    $soal = \App\Models\Soal::with('surat.juz')->findOrFail($id);
    return response()->json([
        'id'            => $soal->id,
